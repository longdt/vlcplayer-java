...
import com.solt.mediaplayer.vlc.remote.VolumeListener;
...
public class FullScreenControls {
	...
	public FullScreenControls(MPlayerFrame player,Shell parent) {
		...
		GC gc = new GC(parent);
		textFont = FontUtils.getFontWithHeight(parent.getFont(),gc ,TIME_LABEL_HEIGHT ,SWT.BOLD);
		gc.dispose();
		...
		Listener fullScreenListener = new Listener() {
			public void handleEvent(Event event) {
				...
				display.asyncExec(new Runnable() {
					...
					public void run() {
						if(!player.isDisposed() && player.getFullscreen()) {
