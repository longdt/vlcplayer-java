...
AESemaphore 
{
	...
	reserve()
	{
		if ( !reserve(0)){
			...
			System.out.println( "AESemaphore: reserve completed without acquire [" + getString() + "]" );
		}
	}
		...
		synchronized(this){
			...
			if ( dont_wait == 0 ){
				...
				try{
						...
						while( true ){
							...
							if ( total_reserve == total_release ){
								...
								if ( spurious_count > 1024 ){
									...
									System.out.println( "AESemaphore: spurious wakeup limit exceeded" );
					...
					System.out.println( "**** semaphore operation interrupted ****" );
